...
fun MacrobenchmarkScope.startActivityAndWaitScreen(screenTag: String) {
    ...
    Log.d("MacrobenchmarkActions", "Waiting for screen with tag: $screenTag")
    ...
    if (!isScreenReady) {
        ...
        try {
            ...
            Log.e("MacrobenchmarkActions", "Screen with tag '$screenTag' not found. UI Hierarchy:\n${hierarchyDump.toString("UTF-8")}")
        } catch (e: Exception) {
            Log.e("MacrobenchmarkActions", "Screen with tag '$screenTag' not found. Failed to dump UI Hierarchy: ${e.message}")
        } finally {
            try {
                hierarchyDump.close()
            } catch (e: Exception) {
    ...
    Log.d("MacrobenchmarkActions", "Screen with tag '$screenTag' found.")
}

fun MacrobenchmarkScope.tapAddNewEntry() {
    val fabTag = "add_entry_fab"
    Log.d("MacrobenchmarkActions", "Attempting to find FAB with testTag: $fabTag")
...
//            if (device.wait(Until.clickable(), 5_000)) { // Additional wait for clickability
            Log.d("MacrobenchmarkActions", "FAB is now confirmed clickable.")
            fab.click()
            Log.d("MacrobenchmarkActions", "FAB clicked.")
...
//            } else {
//                Log.e("MacrobenchmarkActions", "FAB with tag '$fabTag' found but did not become clickable within the additional wait time.")
...
//            }
        ...
        Log.e("MacrobenchmarkActions", "FAB with tag '$fabTag' not found after 15 seconds (Until.hasObject returned false).")
        // Consider dumping UI hierarchy here for debugging
        // Log.e("MacrobenchmarkActions", "Current UI Hierarchy: ${device.dumpWindowHierarchy()}")
